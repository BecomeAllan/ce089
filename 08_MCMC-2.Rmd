...
subtitle: "Algoritmo de Metropolis-Hastings"
...
opts_chunk$set(fig.path = "figures/08_MCMC-2/")
```
...
sequência de probelas menores que são mais fáceis de resolver (e.g.
amostrador de Gibbs).
...
processo no tempo $t$, dados todos os outros valores da cadeia, é igual
...
determina se a cadeia se move de um estado para outro é chamade de
...
$$
Por exemplo, se estamos no PR, a probabilidade de ir para SC é 0.4.

Suponha que inicialmente estamos em SC comprobabilidade 1. Então a
...
- Basic limit theorem de peng
- Propriedades desejadas para MCMC
- Convergencia da cadeia conforme Kinas (outra forma de obter a
  estacionaria)
...
Considere o examplo da aula sobre o algoritmo de aceitação-rejeição,
onde deseja-se gerar valores de uma distribuição $\text{Beta}(\alpha =
2.7, \beta = 6.3)$, com uma distribuição proposta $\text{U}(0,1)$.
