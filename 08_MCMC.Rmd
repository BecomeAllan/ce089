...
(X_1, X_2) \sim \mathcal{N}_2 \left(
  ...
  \begin{bmatrix} \sigma_1 & \rho \\ \rho & \sigma_2 \end{bmatrix}
  \right)
$$
...
A distribuição conjunta  de $X$ e $\theta$ é
...
A correspondente distribuição condicional de $X|\theta$ foi definida
acima, enquanto que a condicional $\theta|x$ é

$$
\begin{aligned}
f(\theta|x) &\propto f(X|\theta) \cdot f(\theta) \\
  &\propto \frac{\Gamma{(\alpha+\beta)}}{\Gamma{(\alpha)}\Gamma{(\beta)}}
...
Portanto, $f(\theta|x) \sim Beta(\alpha^* = \alpha+x, \beta^* = \beta +
n - x)$
...
A distribuição marginal de $\theta$ é $\text{Beta}(\alpha, \beta)$. Já a
distribuição marginal de $X$ é menos comum, e é chamada de beta-binomial
