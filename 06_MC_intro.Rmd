...
opts_chunk$set(fig.path = "figures/06_MC_intro/")
```
...
\begin{align*}
\theta &= \frac{1}{f(x)} \int_a^b g(x) f(x) dx \\
...
m.theta.hat
```
e a variância relacionada ao estimador pode então ser calculada como
...
```{r}
## Media e variancia
c(m.theta.hat, v.theta.hat, se.theta.hat)
...
mais amostras onde a contribuição para a integral é maior, ou
...
\begin{align*}
f_1(x) &= 1, \quad 0<x<1 \\
f_2(x) &= e^{-x}, \quad 0<x<\infty \\
f_3(x) &= (1+x^2)^{-1}/\pi, \quad -\infty<x<\infty \\
f_4(x) &= e^{-x}/(1-e^{-1}), \quad 0<x<1 \\
f_5(x) &= 4(1-x^2)^{-1}/\pi, \quad 0<x<1 \\
\end{align*}
...
- $f_1$ e $f_2$ possuem domínio maior, o que irá contribuir com muitos
  zeros (ineficientes)
- $f_2$ é uma Exponencial com $\lambda = 1$
- $f_3$ é uma Cauchy padrão
- $f_4$ e $f_5$ não são funções conhecidas, por isso precisamos usar o
...
lines(x, f2, col = 3, lwd = w)
lines(x, f3, col = 4, lwd = w)
lines(x, f4, col = 5, lwd = w)
lines(x, f5, col = 6, lwd = w)
legend("topright", legend = c("g", 1:5),
       col = 1:6, lwd = w, inset = 0.02)

plot(x, g, type = "l", main = "", ylab = "g(x)/f(x)",
     ylim = c(0,3.2), lwd = w)
lines(x, g/1, col = 2, lwd = w)
lines(x, g/f2, col = 3, lwd = w)
lines(x, g/f3, col = 4, lwd = w)
lines(x, g/f4, col = 5, lwd = w)
lines(x, g/f5, col = 6, lwd = w)
...
x <- rexp(m, 1)   # usando f2
fg <- g(x) / exp(-x)
theta.hat[2] <- mean(fg)
se[2] <- sd(fg)

x <- rcauchy(m)   # usando f3
...
u <- runif(m)    # f5, pelo método da inversa
x <- tan(pi * u / 4)
fg <- g(x) / (4 / ((1 + x^2) * pi))
theta.hat[5] <- mean(fg)
se[5] <- sd(fg)
...
```{r}
tab <- data.frame("$\\hat\\theta$" = theta.hat,
                  "$\\hat{EP}(\\hat\\theta)$" = se/sqrt(m),
                  check.names = FALSE,
                  row.names = paste0("$f_", 1:5, "$"))
knitr::kable(tab) |>
...
Os resultados indicam que $f_4$, e possivelmente $f_5$ produzem as
menores variâncias entre as funções propostas, enquanto que $f_3$ possui
a maior variância.

<!-- Note que $f_0$, a uniforme, possui erro padrão maior. -->
