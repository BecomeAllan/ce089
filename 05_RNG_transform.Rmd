...
(tab <- cbind("Gerado" = prop.table(table(x)), "Teórico" = p))
apply(tab, 2, sum)
## Comparação gráfica
par(mfrow = c(1, 2))
...
## Compara a proporção de valores gerados com a prob. teórica
(tab <- round(cbind("Gerado" = prop.table(table(x)), "Teórico" = p), 3))
apply(tab, 2, sum)
## Comparação gráfica
par(mfrow = c(1, 2))
plot(prop.table(table(x)), xlab = "X", ylab = "P[X]")
...
<div class="panel-heading">Relações trigonométricas</div>
<div class="panel-body">
```{r echo=FALSE, out.width='60%'}
knitr::include_graphics("img/PolarCoordinates.png")
```
</div>
</div>
...
hist(rr)
plot(ecdf(rr))
FFr <- function(r) {1 - exp(-r^2/2)}
curve(FFr(x), 0, 4, add = TRUE, col = 2)
```
...
o uso de funções transcendentais como seno e cosseno.
...
<div class="panel panel-primary">
<div class="panel-heading">Aproximando o  valor de $\pi$</div>
<div class="panel-body">
Note que na relação
$$
...
o algoritmo computacionalmente.
...
da distribuição normal. Esta função usa um algoritmo chamado de
...
zz <- rnorm(1000)
## Usando o método polar implementado aqui
kk <- polarmethod(1000)
```
...
plot(ecdf(kk), col = 5, add = TRUE)
curve(pnorm(x), add = TRUE, col = 2)
legend("right",
       legend = c("Box-Muller (R)", "Box-Muller (Implementado)",
                  "Inversion", "Polar (Implementado)",
                  "Teórica"), cex = 0.8,
       col = c(1, 3, 4, 5, 2), lty = 1, bty = "n")
```
