...
Na verdade, **poderíamos** pensar em uma aproximação **grosseira**,
supondo que vamos amostrar de $X \sim \text{U}(2, \infty)$, onde podemos
aproximar o valor de $\infty$ por um número grande. Por exemplo:
...
podemos pensar que seria mais fácil se pudessemos obter uma **amostra**
da distribuição de interesse. A partir desta **distribuição**, qualquer
cálculo necessário pode ser derivado facilmente.
...
aleatórios. A mesma ideia será usada aqui, mas agora **usaremos a
amostra gerada da distribuição de interesse para resolver problemas de
integração**.
...
Usando um valor fixo apenas para ver o gráfico
...
curve(M * dexp(x, 0.5), add = TRUE, lty = 2, lwd = 2)
points(x, ua * M * g(x), col = 3)
points(y[u >= r], ur * M * g(y[u >= r]), col = 2)
```

Agora podemos prosseguir com o algoritmo mais geral da seguinte forma:
...
Lembrando que, na **amsotrgem por importância**, temos o seguinte
cenário. Suponha que $X$ seja uma VA com densidade $f(x)$.
Seja $Y = g(X)/f(X)$, então
$$
...
1. (**Amostragem**) Gerar uma amostra de $m$ observações independentes
   da função de importância $f(x)$, $\{x_1, \ldots, x_m\}$.
2. Calcular os **pesos**
$$
...
$$
3. (**Re-amostragem por importância**) Amostrar **com reposição** $k <
   m$ valores de $\{x_1, \ldots, x_m\}$, utilizando os pesos
   $\{w^*(x_1), \ldots, w^*(x_m)\}$, como uma **distribuição discreta de
   probabilidades**.
...
A avaliação da proximidade entre a distribuição alvo $f$, e a função
geradora (ou de importância) $g$ ainda é um tema de estudo em aberto.
West (1993) propôs a medida chamada *Entropia Relativa à Uniformidade*
(ERU), definida por

$$
\text{ERU} = -\frac{\sum w^*(x_i) \log{w^*(x_i)}}{\log{m}}
$$

Quando a distribuição alvo e a proposta são idênticas, então os pesos
serão constantes e iguais a $1/m$, fazendo com que $\text{ERU}=1$ seja o
valolr máximo. À medida que a *entropia* entre os $w^*(x_i)$ aumenta,
$\text{ERU} < 1$.

Para distribuições unidimensionais, valores de $\text{ERU} > 0.9$ são
recomendados. Em casos multidimensionais, valores mais baixos são
aceitáveis. Nesse caso, usando a Uniforme, esse valor foi

```{r}
-sum(w.norm * log(w.norm))/log(m)
```

Usando agora uma $\text{Beta}(2, 3)$ como função de importância.

```{r}
a <- 9.39; b <- 33.67
curve(dbeta(x, a,  b), from = 0, to = 1, col = 4)
curve(dbeta(x, 2, 3), from = 0, to = 1, add = TRUE, lty = 2)
legend("right", legend = c("g(x)", "f(x)"),
       lty = c(1, 2), col = c(4, 1), bty = "n")
```
...
## Define funções
g <- function(x, a, b) dbeta(x, a, b)
f <- function(x) dbeta(x, 2, 3) # usando Beta(2, 3)
## 1. Amostragem
m <- 1e5
x <- rbeta(m, 2, 3)
## 2. Calcula pesos
