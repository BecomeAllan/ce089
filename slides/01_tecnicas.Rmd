...
- Também conhecido por *exception handling* (tratamento de excessão)
]
...
.pull-right-40.code80[
```{r}
lapply(list(b0.boot, b1.boot, s2.boot), summary)
```
]
...
fn <- function(...) {
    res <- try(baskara(...))
    ## if(class(res) == "try-error") c(NA, NA)
    if(class(res) == "try-error") warning("Deu erro seu jaguara")
}
er3 <- fn(0, 2, 1)
er3
```
...
- Verifique os pontos críticos da execução e proponha mudanças para
  otimizar o código.
...
## Para gerar o PDF
xaringanBuilder::build_pdf("01_tecnicas.Rmd")
## Nao funciona
```
