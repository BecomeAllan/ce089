...
$\hat\theta_{(i)}^{\star} = (F_{n-1}(x_{(i)}))$ é chamada de
**estimativa parcial** para $\theta$, e a média das estimativas
parciais, ou seja,
...
\hat{\theta}^{\star} é portanto

$$
\text{EP}_{jack} = \sqrt{\frac{n-1}{n} \sum_{i=1}^n
  (\hat\theta_{(i)}^{\star} - \hat\theta^{\star})^2}
$$

Isso faz com que o algoritmo de Jackkkife seja facilmente automatizado,
...
Este é um exemplo de quando o mẽtodo de Jackknife pode **falhar**. Isso
ocorre quando $\hat\theta$ não é uma função "suave" da amostra, como é o
caso da mediana.
