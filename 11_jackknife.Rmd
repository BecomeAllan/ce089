...
sum((theta.jack - mean(theta.jack))^2)
sum((theta.jack - mean(theta.jack))^2)/n
((n - 1)/n) * sum((theta.jack - mean(theta.jack))^2)
sqrt(((n - 1)/n) * sum((theta.jack - mean(theta.jack))^2))
## Essa foma acima foi feita para concordar com
sqrt(sum((x - xbar)^2)/(n*(n-1)))
## quando \het\theta e a media \bar{x}

sqrt(var(theta.jack)/n)
sum((theta.jack - mean(theta.jack))^2)/(n*(n-1))
sqrt(sum((theta.jack - mean(theta.jack))^2)/(n*(n-1)))
sqrt(sum((x - xbar)^2)/(n*(n-1)))
