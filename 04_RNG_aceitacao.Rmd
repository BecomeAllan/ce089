...
$f$ é chamada de **distribuição proposta** (*proposal*).
...
Vamos considerar como $g$ (a distribuição proposta) a densidade de uma
uniforme entre -1 e 1. Então se a base é 2, a altura deve ser 0.5 para
...
text(.15, 1, bquote(f(y) / M %.% g(y) == .(round(r, 3))), pos = 4)
```
...
Assim como no exemplo anterior, podemos visualizar os pontos amostrados
...
estamos desperdiçando a maior parte do tempo computacional amostrando
...
Suponha que deseja-se gerar valores de uma VA $X \sim \text{N}(0, 1)$ a
partir de uma VA $Y \sim \text{Cauchy}(0, 1)$.
